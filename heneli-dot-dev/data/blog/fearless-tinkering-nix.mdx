...
This is the second part of [Fearless Tinkering is Functional](./fearless-tinkering-is-functional):
...
The classic advice to "look both ways before crossing" is extremely applicable to software. Programmers producer _and_ consume software. There being multiple directions makes decisions around maintainability a tricky and indivualized calculus. Further, it's difficult to establish standards or policies without having transparent views both upstream and downstream. Mutability is a leaky abstraction, forcing problems downstream until someone confonts it.

Sharing is intrinsic to Nix's programming model. Its model for treating internal and external dependency relations is explicit and uniform. Package users can be sure that package authors aren't implicitly offloading upstream problems on to them, while authors can expect the same of their dependencies. As identically built artifacts are placed in identical locations, the sum cost of immutability is negligible. These properties enable programmers to fearlessly contribute, distribute, and consume software.
...
This makes it clear that programmers need better tools for safely distributing and consuming external software. These should be simple enough that anyone (independent of specialized security researchers) can audit their supply chain. Nix is no silver bullet, but it does provide supply chain tranparency. Every artifact in a Nix environment has a traceable origin, a property otherwise known as [_**provenance**_](https://zero-to-nix.com/concepts/provenance). Fetching software over the internet is made safe by requiring that the received contents have a matching hash to that of the expected contents.
