...
[^6]:
    ...
    One workaround is to provide a false implementation of typeclass operations as a record, and pass it around manually like a typeclass would if it was handling the plumbing. This is the approach taken in Figure 2. That example's false implementation turned out to be problematic as it relied on non-generated values. Since my earlier point was more the ability to co-locate testing and instances within documentation, I've decided to leave the example as is (for now).

    Two interesting things to explore would be a) making law-checking without a valid instance more ergonomic and b) making a fancier anti-instance wrapper that takes a counterexample as input before returning an `Unsatisfiable` constraint.
