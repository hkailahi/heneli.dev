...
The easiest and most common method for dissallowing typeclass operations on a given type is to simply not provide a typeclass instance.

For example, there is no `Monoid` instance provided for the [`First`](https://hackage.haskell.org/package/base-4.18.0.0/docs/Data-Semigroup.html#t:First) datatype in `Data.Semigroup`. `First` does come with a `Semigroup` instance as combining `First` values with `(<>)` obeys the associativity law[^1].
...
`First` cannot have a lawful `Monoid` instance as it can't satisfy the identity law[^1]. This means no value can be supplied to `First` that, when combined a distinct second value, would result in the second value.
...
The release of GHC 8.0.1 in 2016 brought us [`TypeError`](https://downloads.haskell.org/~ghc/8.0.1/docs/html/users_guide/glasgow_exts.html#custom-errors). Unlike with omitted instances, `TypeError` allowed for explicit anti-instances.
...
In addition to being explicit, custom type errors allow for user-defined error messages and provide static documentation pointers that can be referenced in discussions.

Arguably one downside of `TypeError` is its implementation as a type family (AKA a type-level function). This provides added flexibility, but requires greater type-level sophistication from users.

While custom type errors have not been especially common in the Haskell ecosystem, there are examples of them being used in very cool ways.[^3]

### Introducing `Unsatisfiable`

The upcoming release of GHC 9.8.1 will bring a new typeclass called `Unsatisfiable`.
...
Anti-instances defined with `Unsatisfiable` look nearly identical to those defined with `TypeError`, but have some additional benefits. Unlike `TypeError`, `Unsatisfiable` is a typeclass and has kind `Constraint`.
...
This results in // TODO - blah blah blah
...
Whether facing an implicit anti-instance by omission or an explicit anti-instance by custom type error, newtypes overcome anti-instances by creating a wrapper type with which fresh instances can be created off of without polluting the underlying type.
