...
[^2]:
    ...
    Here are some classic typeclass heirarchies for implict yet intentionally omitted instances:
    ...
    * [`NumHask`](https://hackage.haskell.org/package/numhask-0.11.0.2/docs/NumHask.html#g:2) (Alternative numeric hierarchy for Haskell)
    ...
    [Counterexamples of Type Classes](https://blog.functorial.com/posts/2015-12-06-Counterexamples.html) explores several anti-instance heirarchies in Purescript with counterexamples.
...
[^6]:
    Testing anti-instances against something like `monoidLaws` in [`quickcheck-classes`](https://hackage.haskell.org/package/quickcheck-classes) or [`hedgehog-classes`](https://hackage.haskell.org/package/hedgehog-classes) won't work. This is a good thing, as those checkers require operations from typeclass instances that we want to assert _don't exist_.

    One workaround is to provide a false implementation of the typeclass operations as a record, and pass it around manually like a typeclass would if it was handling the plumbing. This is the approach taken in the Figure 2. That examples false implementation turned out to be problematic as it relied on any non-generated values. As point was more about co-locating lawful testing and instances within documentation, I've decided to leave the example as is for now.
