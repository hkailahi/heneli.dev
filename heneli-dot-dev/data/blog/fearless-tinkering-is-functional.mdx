...
The entropy of modern software is staggering. Programs inside programs directing separate programs exist, and each layer contains their own digital metropolis. Whether it’s the shear connectors in the bridges, building codes of the plumbing networks, or newest floor plan atop a towering skyscraper, every software artifact reflects a choice made by someone somewhere. The feats of modern software could not be made if every virtual wheel needed reinventing. A global and communal project of shared decisions is harnessed to base, tweak, and publish new constructions.
...
While it might mean tremendous pain in the short term, application programmers can usually throw their hands up and start over from scratch when an insurmountable problem arises in their local environment. If a developer ties their local project repository into an knot, they can blow it away and pull out a fresh copy. Application programmers regularly break their own local environments in ways that would be intolerable anywhere else. Though programs are inseparable from their execution environments, application programmers can approximate control over their state space by optimizing for their own fixed environment.

##### ***Industry's Answer***

How can the software industry bring the advantages of application programming to infrastructure? Containers and Infrastructure as Code (IaC) are popular answers.

##### ***Nix: The New Primitive***
