...
With a greater reliance on _<u>declarative constructs</u>_ (expressions) over _<u>imperative ones</u>_ (statements), functional languages eliminate a number of control flow complexities. Jump-based statements, which break compositionality, are replaced with expressions that can be substituted for their evaluations. Bigger programs can be faithfully decomposed into smaller ones. This allows for the local reasoning of individual parts to hold up when global reasoning is needed about the whole. Thanks to the sole determination of a function’s outputs on its inputs, purely functional programming creates an experience where “what you see is what you get”.
...
>
> - Rúnar Bjarnason, [Constraints liberate, Liberties constrain](https://youtu.be/GqmsQeSzMdw) at Scala World 2015
...
>
> - Bryan Cantrill, [Platform as a Reflection of Values](https://vimeo.com/230142234) at Node Summit 2017
...
>
> - Patrick Thomson, definitions from ["Building Haskell Programs with Fused Effects"](https://youtu.be/vfDazZfxlNs?t=145) at Strange Loop 2019
...
Not all is rosy. The imperative nature of things like Dockerfiles destroys composability and reproducibility.

While containers provide a [minimal degree of isolation](https://fly.io/blog/sandboxing-and-workload-isolation/), there's nothing inherent to "contained" environments ensuring compositionality or reproduciblity.

Putting a declarative interface over leaky abstraction might look simpler for the happy path. structural guarantees

Nix is versatile and expandable. It can fit neatly within existing infrastructure, letting engineers benefit from reproduciblity while using plain Linux and MacOS or [containerization tools like Docker](https://floxdev.com/blog/flox-and-containers). Nix can also entirely subsume infrastructure with things like NixOS, NixOps, and deploy-rs.

#### Fearless Environments

<details>
  <summary>Nix in Action: A Minimal Primer</summary>

  Let's say I wanted to know the dependencies of my local `python` installation.

  Whether installed through Nix's CLI or [declaratively with a configuration file](https://github.com/hkailahi/dotnix/blob/c59b2e2654f8a84cab1ac59ab72e991ffe84ca70/home.nix#L20), any package installed with Nix should will be present in my local Nix database (called the [Nix Store](https://zero-to-nix.com/concepts/nix-store)):
  ```bash
  ...
  One could imagine recursively inspecting ancestral build plans to collect transitive dependencies. Instead, I'll use Nix's CLI to both query the Nix Store for dependency graph and temporarily install the `graphviz` package to visualize it:
  ```bash
...
With Nix, everything is built off expressions. Nix allows us to define reproducible and composable environments.

Nix Store is an append-only graph database that stores components in a file system.

Nix is input-addressable by default. As a result, builds are deterministic.

Separation of evaluation and execution.
- Instantiation evaluation a nix expression. This pure computation results in data structure called a derivation that represents a build plan (derivation).
- Realization build
...
>
> - Julien Urraca, paraphrased from ["From Nix to Docker - Everything You Need to Know About Polygot Package Management"](https://youtu.be/onBQhwuPOAc?t=231) at Open Source 101 2023

Sharing both internally and externally. Free dependency sharing. Best of source and binary distributions.

Input-addressable

Benefits of source and binary distribution. Trustworthy binary caches.
...
[^11]: [So fearless](https://github.com/NorfairKing/haskell-WAT)...
    ...
    ```
[^12]:
    While I believe Nix is worth exploring for most developers, it's important that interested parties know what they're signing up for. Learning Nix is a significant undertaking. The explicit, build-from-source model means that package authors are forced to confront essential complexity up front rather than paying off limitations or tech debt as required. This commonly manifests from the bizarre assumptions that popular software make about their host environment and toolchain. Fortunately for end-users, many pre-defined packages exist and are available for modification or cached download. In fact, the primary [Nix's primary package repository](https://search.nixos.org/packages) is the largest repository of "fresh packages"  available [by a wide margin](https://repology.org/repositories/graphs). For everything else, end-users can use provided ["fetchers"](https://nixos.org/manual/nixpkgs/stable/#chap-pkgs-fetchers) to shorcut the build-from-source model while still benefitting from pinned dependencies. These fetchers allow callers to retrieve artifacts from various sources and verify their contents against a hash. Similar to `git`, Nix's underlying model is much simpler to grasp than the current interfaces for interacting with it. Continuous contributions by generous individuals are made towards reflecting this simplicity and polishing end-user interfaces.

    Fortunately, there's been a surge of beginner-friendly tools, like [`flox`](https://floxdev.com/blog/homebrew), [`devenv`](https://devenv.sh/), [`fleek`](https://getfleek.dev/), and [`devbox`](https://www.jetpack.io/devbox/), that allow newcomers to get the benefits of Nix immediately. These services build on top of Nix to provide access to things like package management, container development, and package/environment distribution.
