...
Approaches that meet individual critiques with individual solutions are prone to local maxima. Breakthroughs often require recognition of the systemic complexities underpinning individual ones. Ideal tools are dedicated to proactively sussing out fundamental weakness, rather than reactively catering only to symptoms.[^9] Hence, selecting tools by their <u>**structural guarantees**</u> is about <u>using one's **values** to elect the ways their software _can’t_ go wrong</u>. A fearless tinkerer goes wrong, strong!
...
Much more important than any specific technology are the ideas that underlie them. Demonstrating a market for fearless approaches to the critical pain points of our daily work will help future technologies make accessible the relevant structural guarantees.
...
For many developers, infrastructure is an uncomfortable area. Whether it’s the state of a cluster on `us-east-2` or a configuration outside one’s project repository, reasoning about [action at a distance](https://en.wikipedia.org/wiki/Action_at_a_distance_%28computer_programming%29) can be treacherous. Further, the stakes are often high when managing software deployments. A tangled web of mutual, implicit dependencies means that modifying one’s system could also mean getting stuck in an invalid state. Tolerating a “working” yet unsatisfactory environment is frequently preferable to the risks of transitioning to something better. Even the modern practice of automated upgrades for existing dependencies is [fraught](https://arxiv.org/abs/2109.11921v1). As experimentation with alternatives is difficult, iterating is normally restricted to when one’s hand is forced or the alternative has already been intimately understood. Tools for managing remote environments tend to remain unfamiliar as the consequences of trying them out might yield an expensive `$CLOUD_PROVIDER` bill. A horrible culmination of all these frictions is regularly found in operational firefighting. Operators are tasked with debugging a live system whose interface, tools, and symptoms are typically only practiced against when problems occur. Even in organizations with significant platform investments, the exploratory surface of test deployments rarely reflect the time and space pollution built up across all contributing local developer and remote production environments.

Application programming, in contrast, can give developers a sense of power and ease. Assuming a working environment, the program behavior and outputs of a project are more locally observable. Equipped with a test harness, REPL, and breakpointing facilities, an interactive view of one’s program environment, including variables and available operations, can be explored. An application programmer can further wield static analyzers, linters, and all their most familiar tools to iteratively coax their programs into the desired shape.
...
- function, defined with a **type-level eDSL**, **interpreted by translating polymorphic functions into concrete functions** computing arguments (final encoding, [finally tagless](https://youtu.be/JxC1ExlLjgw))
