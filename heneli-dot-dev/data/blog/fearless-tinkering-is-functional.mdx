...
Nix is **input-addressable** by default. This means that ... and ... (further explored in next section).
- Nix guarantees that packages will be built with the same inputs. Build outputs aren't necessarily deterministic as Nix can't make the
While it can't make non-deterministic software deterministic
Nix can't guarantee 'bit-for-bit' reproducibility

Work is being done to support output-reproducibility **content-addressable** by default. This would remove various avoidable recompilation through [_early cutoff_](https://www.microsoft.com/en-us/research/uploads/prod/2018/03/build-systems.pdf).
...
<details>
  ...
  OCI images can be created and manipulated with the `dockerTools` module provided by `nixpkgs`. Here is an example from the [`nixpkgs` Manual](https://nixos.org/manual/nixpkgs/stable/#sec-pkgs-dockerTools) of a simple nix expression for creating a container with the [`hello`](https://www.gnu.org/software/hello/manual/hello.html) binary:
  ...
  Further documentation and examples can be found in the [Nixpkgs Manual](https://nixos.org/manual/nixpkgs/stable/#sec-pkgs-dockerTools). For optimized containers, see [Optimising Docker Layers for Better Caching with Nix](https://grahamc.com/blog/nix-and-layered-docker-images/), [A faster `dockerTools.buildImage` prototype](https://lewo.abesis.fr/posts/nix-build-container-image/), and [`nix2container`](https://github.com/nlewo/nix2container).
</details>
...
<details>
  ...
  Like Swift protocols or Rust traits, type classes in Haskell let programmers associate named set of operations with types. These association of behaviors are enabling by providing, inheriting, or generating implementations of operation set on a given type.

  For example, let's say I created a `PrimaryColor` type whose values can be `Red`, `Yellow`, or `Blue`. In order to make `PrimaryColor`s displayable or comparable, I can provide "instances" of the `Show` and `Eq` type classes:
  ```hs
  ...
  Haskell can generate instances automatically for a given type based on the instances of the type's fields instances, the type's shape, the type classes' default implementations, or by borrowing instances from an identitically-shaped type.
  ...
  The `Eq` instance generated above uses the underlying `Eq` instances on `String` and `Int` to ensure that two `User`s are equal if and only if they have the same `name` and `age`. In addition, we know at a glance that `User`s can be ordered, displayed, serialized and deserialized to JSON or CSV, and stored in a database.
...
<details>
  <summary>Property Testing with QuickCheck</summary>
</details>

<br />

<details>
  <summary>Lock-Free Concurrency with Software Transactional Memory</summary>
</details>
