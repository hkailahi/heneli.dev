...
<details>
  ...
  A quick way to that verify a package was actually installed via Nix is to trace its location back to the [Nix Store](https://zero-to-nix.com/concepts/nix-store):
  ```bash
  ...
  One could imagine recursively inspecting ancestral build plans to collect transitive dependencies. Instead, I'll use Nix's CLI to directly query the full dependency graph (called a [transitive closure](https://shopify.engineering/what-is-nix)) and temporarily install `graphviz` to visualize it:
  ```bash
  ...
  Alternatively, I could explore these dependencies interactively with the `nix-tree` file browser.
  ```bash
  $ nix-tree $(which python)
  ```
  ![Python Dependency Graph](/static/images/blog/nix_tree_python.png)

  Let's say I wanted to distribute this dependency information to others in a commonly understood format. Like ingredient labels on food packaging, [SBOMs](https://about.gitlab.com/blog/2022/10/25/the-ultimate-guide-to-sboms/) are a standardized way of providing supply chain transparency for software. Below, I'll use [`sbomnix`](https://github.com/tiiuae/sbomnix) and a temporary `visidata` installation to graphically inspect the ingredients of my `python` package:
  ```bash
