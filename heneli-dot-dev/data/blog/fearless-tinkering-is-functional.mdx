...
My answer is yes. While the above critiques have merit, I don’t believe that any of them are inherent. In fact, I’ll argue that the functional paradigm already has best-in-class learnability thanks to **fearless tinkering**.[^4]
...
With a greater reliance on _<u>declarative constructs</u>_ (expressions) over _<u>imperative ones</u>_ (statements), functional languages eliminate a number of control flow complexities. Jump-based statements, which break compositionality, are replaced with expressions that can be substituted for their evaluations. Bigger programs can be faithfully decomposed into smaller ones. This allows local reasoning on individual parts to hold up when global reasoning is needed about the whole. Thanks to the sole determination of a function’s outputs on its inputs, purely functional programming creates an experience where “what you see is what you get”.

Functional languages address program volume by _<u>making decomposition mechanical</u>_. Programs and their domains can be stripped down to their essence. For instance, verbose wrangling code can be collapsed into neat reusable patterns using higher-order functions and composition. Typed functional languages use various polymorphisms to replace infinitely-many functions over one domain with an economy of functions over infinite domains. Typed functional languages also reduce cognitive volume by offloading the rote calculations of dynamism onto programmer’s better-suited counterparts in computers. As elaborated in the following section, relegating implementation details to the relevant semantic layer makes declarative APIs naturally terse. Judicious use of abstraction capabilities assists in uncovering rock-solid, lean foundations for APIs and wider ecosystems.
...
Approaches that meet each critique with an independent solution are prone to local maxima. Breakthroughs often require recognition of the systemic complexities underpinning individual occurrences. Ideally, tools should proactively seek out fundamental weaknesses, rather than waiting to reactively patch symptoms.[^9] Hence, selecting tools by their **structural guarantees** is about using one's **values** to elect the ways in which their software _can’t_ go wrong. A fearless tinkerer knows they can be wrong, strong!

In this section, I’ll introduce two functional technologies and illustrate how their structural properties enable **fearless tinkering**. These alone won’t do functional ecosystems justice.[^10] Even amongst the tools mentioned, there are plenty of assurances I won't cover. Conversely, I won't spend much time on their structural deficiencies, vestigial quirks, and footguns[^11]. Much more important than any specific technology are the ideas that underlie them. Calling out the need for fearless approaches to the pain points of daily work will help future technologies incorporate the relevant structural guarantees.


### Nix & NixOS

The entropy of modern software is staggering. Programs inside programs directing separate programs exist, with each layer containing their own digital metropolis. Whether it is the shear connectors in the bridges, building codes of the plumbing networks, or newest floor plan atop a towering skyscraper, every software artifact reflects a choice made by someone somewhere. The feats of modern software could not be made if every virtual wheel needed reinventing. A global and communal project of shared decisions is harnessed to base, tweak, and publish new constructions.
...
Nix has <u>_**transactional semantics**_</u>. Whether installing a new package or upgrading one's system, changes through Nix are <u>_**atomic**_</u>. This means all changes happen at once or not at all. Even if a deployment operation is interrupted, users are never left in a partially updated state. If a completed upgrade causes issues, one can immediately roll back to the previous configuration. As a result, iterating on one's environment can be done without commitment. Users can trial packages with temporary installations that don't pollute their environments. Build-time dependencies and binaries for one-off commands can be pruned  beyond their usage. The combination of rollbacks and version control let's one recreate their environment from any point in the past. If one no longer needs a component, they can confidently remove it without fear of changing their mind. Package hoarding is made unnecessary and configurations can be made minimal.

As each package is stored in a <u>_**unique**_</u> path, different versions or variants of a package can coexist without conflict. Users can create and modify multiple developer environments for multiple projects without breaking one another. Users can decide the granularity of their environments like being project, user profile, and system level. One could define a completely isolated developer environment for a project that contained all tooling used like local commands, a fully-configured editor, a local database, and more. Alternatively, one could factor out general system configuration into a top-level repository, and have projects inherit various configurations explicitly.

The Nix expression language is <u>_**purely functional**_</u> and thus <u>_**immutable**_</u> by default. While Nix exhibits observable mutability through top-level views, modified references can be reliably reverted thanks to the immutability of the underlying component store. For example, a user's current environment is simply a component in the Nix Store. Changing a user's environment will result in a new environment in the Nix Store that is then pointed to by the user's current view. Environments that share the same lineage are called 'generations', and can be 'rolled back' using Nix's CLI to recover a previous top-level view.

Every artifact in a Nix environment has a traceable origin, otherwise known as [_**provenance**_](https://zero-to-nix.com/concepts/provenance). Fetching software over the internet is made safe by requiring that the received contents have a matching hash to that of the expected contents. As detailed below, this same approach can be used to enable binary caches with the same compliance benefits of building from source.
...
<u>_**Reproducibility**_</u> enables reliable distribution of Nix-based infrastructure. Nix enables [<u>_**cloud builds**_</u>](https://www.microsoft.com/en-us/research/uploads/prod/2018/03/build-systems.pdf), meaning that native builds are amortized across machines. Whether from [Nixpkgs](https://search.nixos.org/packages), a CI server, or a developer machine, packages can be built once and shared anywhere. Nix can recover sharing benefits within separate infrastructure like [optimally-layered docker images](https://grahamc.com/blog/nix-and-layered-docker-images/) and [pinned Bazel builds](https://nix-bazel.build/). Additionally, Nix has [world-class support for cross compilation](https://nix.dev/tutorials/cross-compilation#cross-compilation).
...
<details>
  ...
  Outside of garbage collection, any package installed via Nix will be present in one's local Nix database (called the [Nix Store](https://zero-to-nix.com/concepts/nix-store)). This fact let's me verify that I installed the package via Nix by finding it in the Nix Store:
  ```bash
  $ readlink $(which python)
  /nix/store/7fkcip5klr0gksrjgdb3xf0wiz89mdak-python3-3.11.3/bin/python
  ```

  One route for finding dependencies would be to inspect the package's build plan (called a [derivation](https://zero-to-nix.com/concepts/derivations)) to find direct dependencies and their build plans:
  ```bash
  ...
  One could imagine recursively inspecting ancestral build plans to collect transitive dependencies. Instead, I'll use Nix's CLI to query the full dependency graph directly (called the [transitive closure](https://shopify.engineering/what-is-nix)) and temporarily install `graphviz` to visualize it:
  ```bash
  ...
  In addition, we can audit it by generating SBOMs with [`sbomnix`](https://github.com/tiiuae/sbomnix):
  ```bash
...
Refactoring in Haskell is fearless, thanks to the language's static type system and the principle of equational reasoning, which posits that expressions in your code can be substituted with their corresponding values without changing the program's behavior. When you refactor your code, the compiler and type system ensure your modifications maintain the same behavior as before. Even if changes span across numerous files, as long as it compiles, you can be confident it will work correctly.

The Haskell type system makes your data model explicit, enhancing the expressiveness and eliminating ambiguity often found in loosely typed languages. Types act as a form of documentation and ensure that you can never misuse keys in a database table or make other similar errors. Haskell's emphasis on strong and static types supports type-safe operations, aiding refactoring by providing better guarantees at compile time.

Fearless refactoring is further aided by Haskell's emphasis on data structures over code. As famously modernized from Fred Brooks' quote, "Show me your data structures, and I won’t usually need your code; it’ll be obvious." In Haskell, well-designed and well-typed data structures can tell more about the program than the actual code, contributing significantly to making refactoring less risky and more intuitive.

<details>
  <summary>Refactoring Made Mechanical</summary>

  Translating functions into “generalized” forms is fairly mechanical. For example, one progression could be:
...
Haskell's static and expressive type system, strong emphasis on immutability, and pure functions contribute to making software maintenance in Haskell a more fearless process compared to other languages. These features remove entire classes of common runtime errors like null dereferencing and unchecked pattern matches. They provide robust safeguards that make the maintenance process more predictable and safer, thereby facilitating fearless maintenance.

In a team setting, these features enable more effortless collaboration and effective code reviews. Changes proposed in a pull request can be substantial, but thanks to the type system, code reviewers can trust that the compiler has already performed a significant amount of checks. This mutual trust in types reduces cognitive load, enabling the team to focus on architectural decisions, design, and abstractions, rather than being bogged down by complex code baggage.

Haskell's ecosystem adds to the fearlessness in maintenance. Despite the potential for occasional complications during upgrades due to the community's enthusiasm for change and improvements, the language's shared values and focus on simplicity and reusability make maintenance easier. Also, tools like QuickCheck enable broader test coverage, further enhancing the trust in the system's reliability.

In conclusion, while maintenance is an inevitable part of the software lifecycle, Haskell’s strong typing, functional purity, and the benefits of equational reasoning all contribute to making maintenance not just manageable, but fearless.
...
<details>
  ...
  instance Eq PrimaryColor where
    -- An infix function that takes two colors and returns whether they match
  ...
  instance Show PrimaryColor where
    -- A function that takes a color and outputs its string representation
  ...
  ```hs
  λ> let phil = Person "Phil" 25
  λ> let zoe = Person "Zoe" 42
  λ> phil > zoe
  False
  λ> zoe == zoe
  True
...
<details>
  ...
  Property-based Testing originated from Haskell with the QuickCheck library. Rather than writing unit tests that check the behavior of a function for specific inputs (a la unit tests), property-based tests allow developers to define invariants that the function must hold for any input. The testing library then generates random inputs to test the function thoroughly. This approach provides stronger guarantees and finds edge cases that might have been missed with traditional unit tests, allowing developers to refactor and modify their code fearlessly.
...
Beyond the strict constraints of functional programming, developing the algebra of one’s program, be it a subset of purely functional properties or different constraints, can greatly enhance the reliability and predictability of any paradigm. Many non-functional technologies exploit their inherent algebra to offer their own "fearless usages".

For instance, consider the RESTful operations in the world of web services. The key to "fearless puts" in REST lies in the property of idempotence - the ability to apply an operation multiple times without changing the result beyond the initial application. This principle provides strong guarantees for the execution of read and upsert operations, as engineers can rely on informal reasoning about the absence of POSTs and DELETEs to enhance reliability and predictability.

"Stateless" applications offer a further exploration of algebraic structures in software design. While such applications do have state, it is localized and, thus, unobservable to the outside world. Despite the potential momentary application state, heap state, open database connections, and the existence of unlucky client connections at the moment of failure, stateless applications can be fearlessly terminated and restarted thanks to their reproducible and immutable nature.

However, as Bryan Cantrill points out, few applications are truly "stateless". At the moment of failure, your server will likely have local application state, heap state, open database connections, and unlucky client connections that will go home with a 500. Without a snapshotting mechanism for exploring and debugging true failure states, merely restarting “stateless” application failures might erode the application’s purported safety mechanisms.

In the context of infrastructure, declarative solutions such as Terraform, Pulumi, and CDK empower developers to fearlessly modify systems without complex migrations. By describing the desired state, as opposed to the steps to achieve it (imperative programming), these technologies simplify system management and improve predictability. This concept can be seen with NixOS, where it enables fearless system deployments.

Batch processing systems like MapReduce and Spark leverage the associativity property of their operations for fearless parallelization, allowing developers to focus on their core business logic rather than solving complex distributed systems problems. Similarly, Git offers an imperative interface over an immutable purely functional data structure, empowering developers with fearless sharing and rollbacks.

Overall, embracing algebraic structures in software design can unlock powerful "fearless" capabilities, improving both the predictability and reliability of systems across the technology stack. It offers an additional tool in the toolbox for developers seeking to strengthen their system's guarantees. The algebraic approach is not restricted to functional programming, proving its value in various paradigms, be it declarative infrastructure or stateless application design.
...
- If you’re interested in a functional devops experience, I recommend checking out [Nix](https://nixos.org/).[^13]
...
[^13]:
    While I believe Nix is worth exploring for most developers, it's important that interested parties know what they're signing up for. Learning Nix is a significant undertaking. The explicit, build-from-source model means that package authors are forced to confront essential complexity up front rather than accruing tech debt that can be paid later. Roadblocks commonly manifest from the bizarre assumptions made by various software about their host environment and toolchain. The cruel reality is that many packaging tasks with Nix are actually just sophisticated ways of gluing [bash scripts](https://nixos.org/manual/nixpkgs/stable/#chap-stdenv) together. Fortunately for end-users, many predefined packages exist and are available for modification or cached download. In fact, the [Nix ecosystem's primary package repository](https://search.nixos.org/packages) contains the largest number of "fresh packages" [by a wide margin](https://repology.org/repositories/graphs). For everything else, end-users can use provided ["fetchers"](https://nixos.org/manual/nixpkgs/stable/#chap-pkgs-fetchers) to shortcut the build-from-source model while still benefiting from pinned dependencies. These fetchers allow callers to retrieve artifacts from various sources and verify their contents against a hash. Similar to `git`, Nix's underlying model is much simpler to grasp than the current interfaces for interacting with it. Ongoing contributions by generous individuals are being made towards reflecting this simplicity and polishing end-user interfaces.

    Fortunately, there's been a surge of beginner-friendly tools that allow newcomers to get the benefits of Nix immediately. Some examples include [`flox`](https://floxdev.com/blog/homebrew), [`devenv`](https://devenv.sh/), [`fleek`](https://getfleek.dev/), and [`devbox`](https://www.jetpack.io/devbox/). These services build on top of Nix to provide access to things like package management, container development, and package/environment distribution.
